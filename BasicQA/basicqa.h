...
// Wed Apr 11 19:45:15 2018 by ROOT version 5.34/36
// from TTree cbmsim_reduced/cbmsim_reduced
// found on file: /lustre/nyx/hades/user/parfenov/mpd_new/DST/PicoDst/11gev/mpddst_43615227_10_reduced.root
//////////////////////////////////////////////////////////
...
public :
   ...
   Float_t         eta_mc[2362];   //[n_tracks_mc]
   Float_t         pt_mc[2362];   //[n_tracks_mc]
   Int_t           mother_ID_mc[2362];   //[n_tracks_mc]
   Int_t           PDG_code_mc[2362];   //[n_tracks_mc]
   Float_t         px_mc[2362];   //[n_tracks_mc]
   Float_t         py_mc[2362];   //[n_tracks_mc]
   Float_t         pz_mc[2362];   //[n_tracks_mc]
   Float_t         start_x_mc[2362];   //[n_tracks_mc]
   Float_t         start_y_mc[2362];   //[n_tracks_mc]
   Float_t         start_z_mc[2362];   //[n_tracks_mc]
   Float_t         mass_mc[2362];   //[n_tracks_mc]
   Float_t         energy_mc[2362];   //[n_tracks_mc]
   Long64_t        n_tracks_mpd;
   Long64_t        k_tracks_mpd;
   Float_t         eta_mpd[1215];   //[n_tracks_mpd]
   Float_t         phi_mpd[1215];   //[n_tracks_mpd]
   Float_t         theta_mpd[1215];   //[n_tracks_mpd]
   Int_t           TOF_flag_mpd[1215];   //[n_tracks_mpd]
   Float_t         ZDC_energy_mpd[90];
   Float_t         pid_tpc_prob_electron_mpd[1215];   //[n_tracks_mpd]
   Float_t         pid_tpc_prob_pion_mpd[1215];   //[n_tracks_mpd]
   Float_t         pid_tpc_prob_kaon_mpd[1215];   //[n_tracks_mpd]
   Float_t         pid_tpc_prob_proton_mpd[1215];   //[n_tracks_mpd]
   Float_t         pid_tof_prob_electron_mpd[1215];   //[n_tracks_mpd]
   Float_t         pid_tof_prob_pion_mpd[1215];   //[n_tracks_mpd]
   Float_t         pid_tof_prob_kaon_mpd[1215];   //[n_tracks_mpd]
   Float_t         pid_tof_prob_proton_mpd[1215];   //[n_tracks_mpd]
   Float_t         tof_beta_mpd[1215];   //[n_tracks_mpd]
   Float_t         tof_mass2_mpd[1215];   //[n_tracks_mpd]
   Float_t         dEdx_tpc_mpd[1215];   //[n_tracks_mpd]
   Float_t         chi2_mpd[1215];   //[n_tracks_mpd]
   Float_t         chi2_vertex[1215];   //[n_tracks_mpd]
   Float_t         pt_error_mpd[1215];   //[n_tracks_mpd]
   Float_t         theta_error_mpd[1215];   //[n_tracks_mpd]
   Float_t         phi_error_mpd[1215];   //[n_tracks_mpd]
   Float_t         DCA_x_mpd[1215];   //[n_tracks_mpd]
   Float_t         DCA_y_mpd[1215];   //[n_tracks_mpd]
   Float_t         DCA_z_mpd[1215];   //[n_tracks_mpd]
   Int_t           n_hits_mpd[1215];   //[n_tracks_mpd]
   Int_t           n_hits_poss_mpd[1215];   //[n_tracks_mpd]
   Float_t         signed_pt_mpd[1215];   //[n_tracks_mpd]
   Int_t           centrality_tpc_mpd;
   Long64_t        id_from_mc_mpd[1215];   //[n_tracks_mpd]
   Float_t         p_mpd[1215];   //[n_tracks_mpd]
   ...
   basicqa(TTree *tree=0);
   basicqa(TString inputName, TString outputName);
   virtual ~basicqa();
   virtual Int_t    Cut(Long64_t entry);
   virtual Int_t    GetEntry(Long64_t entry);
   virtual Long64_t LoadTree(Long64_t entry);
   virtual void     Init(TTree *tree);
   virtual void     LoopPID();
   virtual void		LoopDCA();
...
#ifdef basicqa_cxx
basicqa::basicqa(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/lustre/nyx/hades/user/parfenov/mpd_new/DST/PicoDst/11gev/mpddst_43615227_10_reduced.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("/lustre/nyx/hades/user/parfenov/mpd_new/DST/PicoDst/11gev/mpddst_43615227_10_reduced.root");
      }
...
basicqa::basicqa(TString inputName, TString outputName) : fChain(0) 
{
...
Int_t basicqa::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t basicqa::LoadTree(Long64_t entry)
{
...
void basicqa::Show(Long64_t entry)
{
...
Int_t basicqa::Cut(Long64_t entry)
{
